...
let shirtMaterial; // Variable para almacenar el material de la camiseta
...
const defaultshirtColor = 0xFFFFFF; // Color blanco
...
function inicializar() {
    ...
    loader.load('/clothes/3d/jacket.glb', function (gltf) {
        ...
        // Buscar el material con nombre "shirt_Texture"
        gltf.scene.traverse(function (child) {
            if (child.isMesh && child.material.name === "shirt_Texture") {
                shirtMaterial = child.material;
            }
        });
...
color1.addEventListener('click', function () {
    ...
    if (shirtMaterial) {
        shirtMaterial.color.set(0x1C1C1C); // Color rojo
    }
...
color2.addEventListener('click', function () {
    ...
    if (shirtMaterial) {
        shirtMaterial.color.set(0xFFFFFF); // Color blanco
    }
...
color3.addEventListener('click', function () {
    ...
    if (shirtMaterial) {
        shirtMaterial.color.set(0xB40404); // Color rojo
    }
    selectedColor = "Rojo";
    updateSelectedOptions();
});
...
color4.addEventListener('click', function () {
    ...
    if (shirtMaterial) {
        shirtMaterial.color.set(0x0431B4); // Color azul oscuro
    }
...
function resetDefaultColors() {
    if (shirtMaterial) {
        shirtMaterial.color.set(defaultshirtColor);
    }
...
// Función para limpiar las opciones seleccionadas y desactivar los mesh de los decals, si están activados
function limpiarOpciones() {
    ...
    if (shirtMaterial) {
        // Establecer el color por defecto del material de la camiseta
        shirtMaterial.color.set(defaultshirtColor);
    }
    ...
    if (usuario) {
        ...
        opcionesRef.once('value')
            .then(snapshot => {
                ...
                if (opcionesGuardadas && opcionesGuardadas["Camisetas"]) {
                    contadorSelecciones = Object.keys(opcionesGuardadas["Camisetas"]).length;
...
function guardarOpciones() {
    ...
    if (usuario) {
        const database = firebase.database();
        const opcionesRef = database.ref(`usuarios/${usuario.uid}/Productos/Seleccionados/Camisetas`);

        const nuevoNombrecamiseta = `Camiseta ${contadorSelecciones}`;

        opcionesRef.child(nuevoNombrecamiseta).set(selectedOptions)
            .then(() => {
